    ...
    return (
        <div className={cardStyles}>
            <div className={css`height: 100%; padding-left: 20px;`}>
                <Link className={css`text-decoration: none;`} to={`/resource/${properties.cluster.id}`}>
                    <Heading className={headingStyles} size="medium">{properties.cluster.name}</Heading>
                </Link>
                ...
                <div className={css` margin-top: 1.1rem; display: flex;`}>
                    ...
                    <BodyShort className={css`font-size: 16px;`}><b>{properties.cluster.members.find((am) => am.navIdent === properties.navIdent)?.roles.map((r: TeamRole) => intl[r]).join(', ')}</b></BodyShort>
                </div>
            </div>
