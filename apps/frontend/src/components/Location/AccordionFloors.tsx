...
const AccordionFloors = (props: AccordionFloorsProps) => {
    const { locationCode, section, floorList } = props
    ...
    return (
        <>
            ...
            >
                {section.subLocations && floorList.map((floor: LocationSimple) => (
                    <Panel
                        key={floor.code}
                        title={<AccordionTitle title={floor.description} />}
                    >
                        {!loading && currentTeamList && (
                            <Block width="100%" display="flex" flexWrap justifyContent="space-between">
                                {currentTeamList.length > 0 ? currentTeamList.map((team: ProductTeam) => <TeamCard team={team} />)
