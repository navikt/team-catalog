  ...
  return (
    <Block width="100%">
      <ul style={{ paddingInlineStart: 0 }}>
        ...
        {editIndex < 0 && (
          <ListItem overrides={{ Content: { style: { height: 'auto' } } }}>
            <Block width={'100%'}>
      ...
      <Block display="flex" justifyContent="space-between">
        <Block>
          <Button kind="tertiary" type="button" icon={faSearch} onClick={() => setNaisMembers(!naisMembers)}>
            Foreslå nais medlemmer
          </Button>
        </Block>
        <Block>
          <Button
            tooltip="Legg til medlem"
            kind="tertiary"
            type="button"
            icon={faPlus}
            onClick={() => {
    ...
    <ListItem overrides={{ Content: { style: { height: 'auto' } } }}>
      <Block width="100%">
        <Block width="100%" display={'flex'} justifyContent={'space-between'} alignItems={'center'}>
          <Block width={'100%'}>
            {editRow && <FormEditMember onChangeMember={props.onChangeMember} member={member} filterMemberSearch={props.filterMemberSearch} />}
...
const Buttons = (props: { hide: boolean; editMember: () => void; removeMember: () => void }) => {
  return props.hide ? null : (
    <>
      <Button type="button" kind="tertiary" onClick={props.editMember}>
        <FontAwesomeIcon icon={faEdit} />
      </Button>
      <Button type="button" kind="tertiary" onClick={props.removeMember}>
  ...
  return (
    <ListItemLabel>
      <StatefulTooltip content={member.navIdent} focusLock={false}>
        {member.fullName && (
          <span>
            <b>{member.fullName}</b> ({intl[member.resourceType!]}) {roles}
          </span>
        )}
        {!member.fullName && (
          <span>
            <b>{member.navIdent}</b> (Ikke funnet i NOM) {roles}
          </span>
        )}
      </StatefulTooltip>
    </ListItemLabel>
