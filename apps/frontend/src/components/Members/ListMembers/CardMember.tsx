          ...
          <StatefulTooltip content={`Nav-Ident: ${member.navIdent}`}>
            <div>
              <RouteLink href={`/resource/${member.navIdent}`} hideUnderline>
                {member.resource.fullName} {resourceTypeNote(member.resource.resourceType)}{' '}
                {member.resource.endDate && moment(member.resource.endDate).isBefore(moment()) && '(Inaktiv)'}
              </RouteLink>
            </div>
          </StatefulTooltip>
        ) : (
          <Block>Personen er ikke funnet i NOM ({member.navIdent})</Block>
        )
      }
      overrides={{
        Root: {
