...
const ProductAreaCardList = (props: ProductAreaCardListProps) => {
  ...
  const getCard = (p: ProductArea) => {
    ...
    return <ProductAreaCard key={p.id} title={p.name} areaType={type} teamSummary={dash?.productAreas.find(pa => pa.productAreaId === p.id)}/>
  }

  const heading = (text: string) =>
    <HeadingXSmall marginTop={0} marginBottom={theme.sizing.scale600} marginLeft={theme.sizing.scale400}>{text}</HeadingXSmall>
