    ...
    <FieldArray name="contactAddresses">
      {(p: FieldArrayRenderProps) => {
        ...
        const push = (v: ContactAddress) => {
          if (!addresses.find((v2) => v2.address === v.address)) p.push(v)
        }
        return (
          <>
            <Block>
...
export const ContactAddressTagList = ({ addresses, remove }: { addresses: ContactAddress[]; remove: (i: number) => void }) => {
...
export const SlackUserSearch = ({ add, close }: AddContactAddressProps) => {
  ...
  return (
    <Block display="flex" flexDirection="column">
      ...
      {loadingSlackId && <CustomSpinner size={theme.sizing.scale800} />}
      {error && (
...
export const AddEmail = ({ added, add: doAdd, close }: AddContactAddressProps) => {
  ...
  return (
    <Block display="flex" flexDirection="column">
      <Block display="flex">
        <Input onKeyDown={onKey} value={val} onFocus={() => setError('')} onChange={(e) => setVal((e.target as HTMLInputElement).value)} onBlur={() => add()} />
