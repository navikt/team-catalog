...
const FieldTeamType = (props: { teamtype: TeamType }) => {
  ...
  const [value, setValue] = React.useState<Value>(teamtype ? [{ id: teamtype, label: intl.getString(teamtype) }] : [])
  return (
    <Field
      name="teamtype"
      render={({ form }: FieldProps<ProductTeamFormValues>) => (
        <Block width={'100%'} maxWidth={'630px'}>
          <Select
            options={Object.values(TeamType).map((tt) => ({ id: tt, label: intl.getString(tt) }))}
            onChange={({ value }) => {
              setValue(value)
