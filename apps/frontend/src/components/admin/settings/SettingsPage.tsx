  ...
  return (
    <>
      <Block alignItems="center" marginTop="1rem">
        <LabelMedium marginRight="1rem" marginBottom="1rem">
          Forsidemelding
        </LabelMedium>
    ...
    <Block marginTop={theme.sizing.scale2400}>
      <Block width={'100%'}>
        <LabelMedium marginRight="1rem" marginBottom="1rem">
          Filtrerte identer
          <StatefulTooltip
            content={<Block>Disse identene vil filtreres bort i all visning i katalogen. Kan skyldes ulike spesielle omstendigheter som f.eks. at personen nylig døde.</Block>}
          >
            <span>
              {' '}
              <FontAwesomeIcon icon={faExclamationCircle} color={colors.blue300} />
            </span>
          </StatefulTooltip>
        </LabelMedium>
        <Input value={val} onChange={(e) => setVal(e.currentTarget.value)} onKeyDown={onKey} onBlur={add} placeholder="Ident" />
      </Block>
      <Block>
        <RenderTagList
          list={props.idents}
...
const AndreOperasjoner = () => {
  ...
  return (
    <Block marginTop={'100px'}>
      <HeadingMedium>Andre operasjoner</HeadingMedium>
      <Block>
        {loading && (
          <Block margin={theme.sizing.scale600}>
            <CustomSpinner size={theme.sizing.scale1200} />
          </Block>
        )}
        <Block display={'flex'}>
          <Button marginRight type={'button'} onClick={resetSync} disabled={loading}>
            Reset sync
          </Button>
          <Checkbox checked={fullSync} onChange={() => setFullSync(!fullSync)} labelPlacement={LABEL_PLACEMENT.right}>
            Full rekjøring av alt
          </Checkbox>
        </Block>
