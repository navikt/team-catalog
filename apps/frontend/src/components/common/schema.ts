...
const roleSchema: yup.SchemaOf<TeamRole> = yup
  .mixed()
  .oneOf(Object.values(TeamRole), errorMessage + ': Rolle')
  .required(errorMessage)

const navIdentSchema: yup.SchemaOf<string> = yup
  .string()
  .matches(/[a-bA-Z]\d{6}/, { message: 'Ugyldig (Invalid) nav-ident' })
  .required()
