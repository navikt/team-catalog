  ...
  return (
    <Block>
      ...
      <Block display={'flex'} justifyContent={'space-between'} flexWrap>
        {floors.map((b, i) => (
          <Button key={i} onClick={() => setFid(b.floorId)} $style={{ marginTop: theme.sizing.scale300 }} kind={b.floorId === fid ? 'secondary' : 'primary'} size="mini">
            {b.name}
          </Button>
        ))}
      </Block>
  ...
  const onUp = (e: React.MouseEvent<SVGElement>) => {
    ...
    if (tar) {
      ...
      if (props.nextId) {
        props
          .nextId()
          .then((id) => create(id, xy.x, xy.y))
          .catch(() => console.debug('cancelled create node'))
      } else {
          ...
          >
            {locations.map((loc) => (
              <Indicator
                ...
                hover={setHighlight}
                fontSize={fontSize}
                teams={teamFor(loc.locationCode)}
                cx={loc.x}
                cy={loc.y}
                rx={teamBubbleSize}
                ry={teamBubbleSize}
                highlight={highlight === loc.locationCode}
              />
            ))}
          </svg>
        </Block>
...
const Indicator = (props: IndicatorParams) => {
  ...
  const textAdjustX = rx * 0.5
  const textAdjustY = fontSize / 2
  return (
    <>
      <text style={{ font: `italic ${fontSize}px sans-serif` }} x={cx - textAdjustX} y={cy + textAdjustY} fill={highlight ? 'red' : 'blue'}>
        {id}
      </text>
      <StatefulTooltip content={<TeamTooltip teams={teams} locationCode={id} />} showArrow focusLock={false}>
        <ellipse
          ...
          rx={rx}
          ry={ry}
          data-location-code={id}
          className="drag"
          stroke={highlight ? 'red' : 'black'}
          strokeWidth={strokeWidth}
          strokeDasharray={highlight ? undefined : 10}
          fill={'red'}
          fillOpacity={highlight ? fillOpacity : 0}
          onMouseOver={() => hover(id)}
          onMouseOut={() => hover(undefined)}
        />
      </StatefulTooltip>
    </>
  )
